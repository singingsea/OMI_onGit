function bin_by_wd_v2_OMI_modified()
use_sum_no2 = true;
type = '1site'; % '2sites'
...
if strcmp(method, 'rotation')
    if use_eccc_no2
        data_path = ['C:\Projects\OMI\plots\Wind_transport\' num2str(Pandora_no) '\rotation_eccc\' ];
        data2_path = ['C:\Projects\OMI\plots\Wind_transport\' num2str(Pandora2_no) '\rotation_eccc\' ];
    else
        data_path = ['C:\Projects\OMI\plots\Wind_transport\' num2str(Pandora_no) '\rotation_knmi\' ];
        data2_path = ['C:\Projects\OMI\plots\Wind_transport\' num2str(Pandora2_no) '\rotation_knmi\' ];
    end
...
elseif strcmp(method, 'twosites_rotation')
    data_path = ['C:\Projects\OMI\plots\Wind_transport\' num2str(Pandora_no) '\twosites_rotation\'];
    load([data_path type 'OMI_Pandora_transport.mat']);
    data = C_all;
...
figure;fig_name = 'Mean_NO2_bin_by_wd_OMI';
boxplot(data_bins.no2);% this is OMI NO2
xticks([1 3 5 7 9 11 13]);
%xticklabels({'15','75','135','195','255','315'});
xticklabels({'0','60','120','180','240','300','360'});
ylabel('OMI NO_2 mean VCD [DU]');
...
legend({'OMI','OMI trop.','OMI start.',['Pandora ' num2str(Pandora_no)]});
xticks([1 3 5 7 9 11 13]);
...
legend({['Pandora ' num2str(Pandora_no) ' - OMI']});
xticks([1 3 5 7 9 11 13]);
%xticklabels({'15^o','75^o','135^o','195^o','255^o','315^o'});
xticklabels({'0^o','60^o','120^o','180^o','240^o','300^o','360^o'});
ylabel('Pandora - OMI NO_2 VCD [DU]');
xlabel('Wind direction');
ylim([-0.35 0.35]);
print_setting(0,save_fig,[plot_path fig_name]);
...
%xticklabels({'15^o','75^o','135^o','195^o','255^o','315^o'});
xticklabels({'0^o','60^o','120^o','180^o','240^o','300^o','360^o'});
ylabel('\delta_r_e_l Pandora - OMI NO_2 VCD [%]');
xlabel('Wind direction');
ylim([-100 100]);
print_setting(0,save_fig,[plot_path fig_name]);
...
%legend({'TropOMI',['Pandora ' num2str(Pandora_no)]});
legend({'OMI AMF','OMI trop. AMF','OMI albedo NO_2'});
xticks([1 3 5 7 9 11 13]);
%xticklabels({'15^o','75^o','135^o','195^o','255^o','315^o'});
xticklabels({'0^o','60^o','120^o','180^o','240^o','300^o','360^o'});
ylabel('NO_2 albedo');
...
%errorbar(1:N_bins,y,y_err);
errorbar(1:(N_bins+1),y,y_err);
ylabel('mean OMI ground pixel number');
%ylim([1 3.1]);
yyaxis right
...
%errorbar(1:N_bins,y2,y2_err);
errorbar(1:(N_bins+1),y2,y2_err);
legend({'OMI mean ground pixel','OMI NO_2 err'});
xticks([1 3 5 7 9 11 13]);
%xticklabels({'15^o','75^o','135^o','195^o','255^o','315^o'});
xticklabels({'0^o','60^o','120^o','180^o','240^o','300^o','360^o'});
ylabel('NO_2 err [DU]');
...
%errorbar(1:N_bins,y2,y2_err);
errorbar(1:(N_bins+1),y2,y2_err);
legend({'OMI trop. AMF','TropOMI trop. NO_2'});
xticks([1 3 5 7 9 11 13]);
%xticklabels({'15^o','75^o','135^o','195^o','255^o','315^o'});
xticklabels({'0^o','60^o','120^o','180^o','240^o','300^o','360^o'});
...
% print_setting(0,save_fig,[plot_path fig_name]);
% 
% fig_name = 'histogram_of_OMI_NO2_err';
% h2 = figure;hold all;
% histogram(data.no2_err./DU,'BinWidth',0.01);ylabel('f');xlabel('OMI NO_2 err [DU]');
% histogram(data2.no2_err./DU,'BinWidth',0.01);ylabel('f');xlabel('OMI NO_2 err [DU]');
% legend({num2str(Pandora_no),num2str(Pandora2_no)});
% xlim([0 0.2]);
% ylim([0 250]);
% print_setting(0,save_fig,[plot_path fig_name]);
% 
% fig_name = 'histogram_of_OMI_NO2_AMF';
% h3 = figure;hold all;
% histogram(data.amf,'BinWidth',0.2);ylabel('f');xlabel('TOMI NO_2 AMF');
% histogram(data2.amf,'BinWidth',0.2);ylabel('f');xlabel('OMI NO_2 AMF');
% legend({num2str(Pandora_no),num2str(Pandora2_no)});
% xlim([0 4.5]);
% ylim([0 250]);
% print_setting(0,save_fig,[plot_path fig_name]);
% 
% fig_name = 'histogram_of_OMI_NO2_AMF_trop';
% h4 = figure;hold all;
% histogram(data.amf_trop,'BinWidth',0.2);ylabel('f');xlabel('OMI NO_2 trop. AMF');
% histogram(data2.amf_trop,'BinWidth',0.2);ylabel('f');xlabel('OMI NO_2 trop. AMF');
% legend({num2str(Pandora_no),num2str(Pandora2_no)});
% xlim([0 4.5]);
% ylim([0 250]);
% print_setting(0,save_fig,[plot_path fig_name]);
% 
% fig_name = 'histogram_of_OMI_NO2_Albedo';
% h5 = figure;hold all;
% histogram(data.albedo_no2,'BinWidth',0.01);ylabel('f');xlabel('OMI NO_2 albedo');
% histogram(data2.albedo_no2,'BinWidth',0.01);ylabel('f');xlabel('OMI NO_2 albedo');
% legend({num2str(Pandora_no),num2str(Pandora2_no)});
% xlim([0 0.1]);
% ylim([0 250]);
% print_setting(0,save_fig,[plot_path fig_name]);
