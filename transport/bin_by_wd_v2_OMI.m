...
if strcmp(method, 'rotation')
    if use_eccc_no2
        data_path = ['C:\Projects\OMI\plots\Wind_transport\' site '\' num2str(Pandora_no) '\rotation_eccc\' ];
        %data2_path = ['C:\Projects\OMI\plots\Wind_transport\' num2str(Pandora2_no) '\rotation_eccc\' ];
%         data_path = ['C:\Projects\TropOMI\plots\Wind_transport\' num2str(Pandora_no) '\rotation_eccc\' ];
%         data2_path = ['C:\Projects\TropOMI\plots\Wind_transport\' num2str(Pandora2_no) '\rotation_eccc\' ];
    else
        data_path = ['C:\Projects\OMI\plots\Wind_transport\' site '\' num2str(Pandora_no) '\rotation_knmi\' ];
        %data2_path = ['C:\Projects\OMI\plots\Wind_transport\' num2str(Pandora2_no) '\rotation_knmi\' ];
%         data_path = ['C:\Projects\TropOMI\plots\Wind_transport\' num2str(Pandora_no) '\rotation_knmi\' ];
%         data2_path = ['C:\Projects\TropOMI\plots\Wind_transport\' num2str(Pandora2_no) '\rotation_knmi\' ];
    end
...
%% filter
tf_time = data.utc_time > datetime('2018-03-01');
data(~tf_time,:) = [];

% tf_time = (data.utc_time > datetime('2018-05-24 00:00')) & (data.utc_time < datetime('2018-05-25 00:00')) ;
% data(tf_time,:) = [];

% TF = (abs(data.x1) <= 10) & (abs(data.y1) <= 10) & (abs(data.arrival_time) <= 1);
% TF = (abs(data.x1) <= 7) & (abs(data.y1) <= 20) & (abs(data.arrival_time) <= 1);
%TF = (data.x1 <= 5) & (abs(data.y1) <= 20) & (abs(data.arrival_time) <= 1);
% TF = (data.x1 <= 10) & (abs(data.y1) <=800) & (abs(data.arrival_time) <= 50);
%TF = (abs(data.x1) <= 5) & (abs(data.y1) <= 10) & (abs(data.arrival_time) <= 1);
TF = (abs(data.x1) <= 14) & (abs(data.y1) <= 30) & (abs(data.arrival_time) <= 1);
data(~TF,:) = [];
...
xticks([1 3 5 7 9 11]);xlim([0 12]);
%xticklabels({'15^o','75^o','135^o','195^o','255^o','315^o'});
xticklabels({'0^o','60^o','120^o','180^o','240^o','300^o'});
...
errorbar(1:N_bins,y2,y2_err);
legend({'TroOMI trop. AMF','OMI trop. NO_2'});
xticks([1 3 5 7 9 11]);
%xticklabels({'15^o','75^o','135^o','195^o','255^o','315^o'});
xticklabels({'0^o','60^o','120^o','180^o','240^o','300^o'});
